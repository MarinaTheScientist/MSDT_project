...
int main()
{
     ...
     */
     ...
     {
        Rect rect; //Мусор(случайные числа в памяти), определил компилятор C++, конструктор выделил память под объект, а деструктор уничтожил объект и освободил занимаемую память. 
    } 
     ...
     */
     ...
     {
        Rect rect(1, 2, 3, 4); //Ошибка произошла из-за того, что когда вызывался объект без параметорв, программа сломалась(нет правил создания объекта без параметров).        
    } 
     ...
     */
     ...
     {
        Rect rect1(1, 2, 3, 4); //вызывается констуктор с 4 параметрами. определил я.
        Rect rect2 = rect1; //вызывается неявный констуктор копирования. определил компилятор.
        Rect rect3(rect1); //вызывается неявный констуктор копирования. определил компилятор.
        //констуктор копирования создает объект как точную копию уже существующего объекта этого же класса.
    } 
     ...
     * Ответ: В общем случае: Очищает динамическую память (delete), закрывает открытые файлы, сетевые соединения и деструкторы вложенных объектов. 
     */ 
...

    {
        ...
        Rect *pR = new Rect(1,2,1,2); // Конструктор с параметрами(в куче).     
        {
            Rect r2(r1); // Конструктор копирования.
            Rect arRect[2]; // Два конструктора по умолчанию для двух объектов массива.
            for(int i = 0; i < 3; i++)
            {
                static Rect r3 (i,i,i,i); // Конструктор с параметрами вызывается 1 раз при i = 0.
                Rect r4(*pR); // Конструктор копирования вызывается 3 раза.
                Rect r5(i,i,i,i); // Констуктор с параметрами вызывается 3 раза.
            } // все r4 и r5 удаляются деструкторами.
        } // Вызываюстя дестукторы сначала для элементов массива arRect[2], а потом для r2.  
        delete pR;  // Вручную удаляется объект в куче по указателю pR.
    } // Сначала удаляется r1, а потом статический обеъкт. 
    ...
    /* {
        Rect r1;
        std::cout << r1.<имя_поля> << std::endl; Ошибка появляется из-за того, что модификатор доступа private ограничивает видимость переменных и позволяет их менять только методам внутри самого класса.
    } */
    ...
    {
       Rect r;
       std::cout << r.get_left() << "; " << r.get_right() << std::endl;
       r.set_all(1, 2, 3, 4);
       std::cout << r.get_top() << "; " << r.get_bottom() << std::endl;
    }
     ...
     * Доступ к скрытым данным(private и protected), связана с конкретным классом, может быть объявлена как virtual.
     */
    ...
    {
        Rect r1(1,2,3,4), r2(5,6,7,8); // 2 конструктора с параметрами. 
        Rect r3 = bounding_rect(r1, r2); // 1 конструктор параметрами.
        print_rect(r3);
    }
...

    {
        ...
        cout << "Ширина: " << r1.get_width() << "\n";
        cout << "Высота: " << r1.get_height() << "\n";
        cout << "Площадь: " << r1.get_square() << "\n";
    ...
    } */
    {
        ...
        Barrel alch(capacity, 0.96);
        Barrel water(capacity, 0.0);

        int i = 0;
        while(alch.get_alc_concentration() > 0.5){
            alch.pour_from(water);
            water.pour_from(alch);
            i++;
        } 
        cout << "Итераций: " << i << "  Концентрация алкоголя: " << alch.get_alc_concentration() << " Концентрация воды: " << water.get_alc_concentration() << "\n";
    }
    ...
    {
        Matrix matrix_ones(2,2,1);
        Matrix matrix_vector(2,1);

        matrix_ones.set(1,1,0);
        matrix_vector.set(0,0,0);
        matrix_vector.set(1,0,1);

        cout << "0 ";
        for (int i = 0; i < 50; i++){
            Matrix result{matrix_ones.multiply(matrix_vector)};
            cout << result.get(0,0) << " ";
            matrix_vector.set(0,0,result.get(0,0));
            matrix_vector.set(1,0,result.get(1,0));
        }
        cout << endl;
    }
    ...
    {
        MyString s1;
        MyString s2("qwerty6767");
        cout << s2.get(3) << "\n";
        s2.set(3, '2');
        cout << s2.get(3) << "\n";
        s2.set_new_string("pfsdpgsdg");
        s1.set_new_string("fdsfs");
        s1.print();
        s2.print();
        s1.read_line();
        s2.read_line();
     ...
     *Комментрий для 2-ой лабы по гиту.
     */
