...
**Цель:** Установить и настроить git\
**Команды:**
```bash
git --version
...
Я создал файл `my_projects` и там же новый репозиторий. Затем в этом репозитории файл `README.md`, директорию `reports` и файл `reports/lab1.md`.
...
*Изменения находяться в индексе и их можно коммитить.*
***
...
Я использовал команду `git restore` и изменения файла `README.md` пропали.
...
Я вывел имя текущей ветки с помощью команды `git branch` -> `main` и создал новую ветку `lab1-1` с помощью команды `git branch lab1-1`. 
Дополнил отчет и закоммитил изменения. 
